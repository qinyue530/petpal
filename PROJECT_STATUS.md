...
后端代码已全部写完，但还未成功启动过。用户需要在本地执行以下步骤：

```bash
cd petpal/server

# 1. 安装依赖（必须在本地执行，沙箱网络不行）
npm install
...
**注意**：`initDb.js` 中也有循环依赖问题（require models 后再 sync），可能需要同样在入口处加载模型。如果报错，需要修复。
...
1. **修复 initDb.js 的模型加载**（同 seed.js 的模式）
2. **本地启动后端**，验证所有 API 接口
3. **前端对接后端 API**：
   - 创建 `client/utils/api.js` — 封装 wx.request，统一 baseURL、token、错误处理
   - 逐页替换 `wx.getStorageSync` 为 API 调用
   - 登录流程对接（wx.login → 后端换 token）
4. **配置微信小程序合法域名**（开发阶段可用「不校验合法域名」）

### 第二优先级：功能完善（产品路线图二期）

5. **支付集成**（微信支付）
6. **消息推送**（模板消息/订阅消息）
7. **图片上传**（对接后端 uploadController）
8. **搜索功能**（后端搜索 API + 前端搜索页）
9. **商家详情页**
...
2. **当前阻塞项**：后端还没成功启动过，`initDb.js` 可能需要修复模型加载
3. **下一步行动**：帮用户验证后端启动 → 修复问题 → 开始前后端联调
4. **不要做的事**：
...
5. **前端所有页面目前使用本地 Storage 模拟数据**，联调时需要替换为 API 调用
